...
### Introduction
The linreg package provides an implementation of Ordinary Least Squares (OLS) linear regression using a reference class object. This vignette will guide you through how to use the linreg class, fit a linear model, and use its associated methods for extracting results such as coefficients, residuals, and predictions.
...
After fitting the model, you can view the coefficients by using the print() method. This will display the intercept and the coefficients for each predictor variable.
...
The summary() method provides a detailed summary of the model, including estimates for each coefficient, standard errors, t-values, and significance levels.
...
### Plotting Residuals
To visually inspect how well the model fits the data, you can plot the residuals against the fitted values using the plot() method.
...
### Conclusion
This vignette provided an overview of the key functionalities of the linreg package, including creating and fitting a linear regression model, viewing model coefficients, generating predictions, accessing residuals, and plotting residuals.
